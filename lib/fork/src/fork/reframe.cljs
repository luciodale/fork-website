        ...
        (let [props (merge props {:state state
          ...
          [component
           {:state state
            ...
            :waiting? (reframe/waiting? (:waiting? db))
            :initial-values (:initial-values props)
            :errors (reframe/errors (:errors @state)
                                    (:server-errors db)
                                    (:external-errors db))
            ...
            :handle-submit #(reframe/handle-submit % (merge props {:db db}))}]))})))
...
(defn set-waiting
  [db path input bool]
  (reframe/set-waiting db path input bool))
...
(defn assoc-server-errors
  [db path input msg]
  (reframe/assoc-server-errors db path input msg))

(defn dissoc-server-errors
  [db path input]
  (reframe/dissoc-server-errors db path input))
...
(defn input-array
  ...
  [component (merge
              ...
              {:array-key name
               ...
               :handle-blur
               (fn [evt idx]
                 (reframe/handle-on-blur-input-array
                  evt props name idx))
               :input-array-errors
               (fn [idx input-key err-ks]
                 (reframe/input-array-errors
                  (merge props {:array-key name})
                  idx input-key err-ks))
               :add
               (fn [evt]
                 (.preventDefault evt)
                 (reframe/add-to-input-array
                  props name))
               :delete
               (fn [evt idx err-ks]
                 (.preventDefault evt)
                 (reframe/delete-from-input-array
